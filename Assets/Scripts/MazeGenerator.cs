...
public class MazeGenerator : MonoBehaviour {
    ...
    private int[,] maze; // 2D array to represent the maze structure
    private List<Vector2> visitedCells; // list of cells that have been visited during maze generation
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (Random.value < monsterFrequency * difficulty) {
                    ...
                    newMonster.transform.position = new Vector3(x * 10, 0, y * 10); // adjust position based on room dimensions
                }
                if (Random.value < chestFrequency * difficulty) {
                    ...
                    newChest.transform.position = new Vector3(x * 10, 0, y * 10); // adjust position based on room dimensions
                }
                if (Random.value < otherObjectFrequency * difficulty) {
                    ...
                    newOtherObject.transform.position = new Vector3(x * 10, 0, y * 10); // adjust position based on room dimensions
                }
            }
        }
    }
    ...
    private List<Vector2> GetUnvisitedNeighbors(Vector2 cell) {
        ...
        if (eastNeighbor.x < width && !visitedCells.Contains(eastNeighbor)) {
            neighbors.Add(eastNeighbor);
        }
        if (westNeighbor.x >= 0 && !visitedCells.Contains(westNeighbor)) {
            neighbors.Add(westNeighbor);
        }
        if (northNeighbor.y < height && !visitedCells.Contains(northNeighbor)) {
            neighbors.Add(northNeighbor);
        }
        if (southNeighbor.y >= 0 && !visitedCells.Contains(southNeighbor)) {
